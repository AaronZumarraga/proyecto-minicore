...
app.get('/notas/progreso2/:idEstudiante', (req, res) => {
    const idEstudiante = req.params.idEstudiante;
    const fechaInicio = req.query.fechaInicio || '1970-01-01';
    const fechaFin = req.query.fechaFin || '9999-12-31';

    connection.query(
        'SELECT Nota FROM notas WHERE Id_estudiante = ? AND Fecha BETWEEN ? AND ? AND Id_nota IN (SELECT Id_nota FROM progreso2 WHERE Id_estudiante = ?)',
        [idEstudiante, fechaInicio, fechaFin, idEstudiante],
        (error, results) => {
            if (error) {
                console.error('Error al obtener las notas del segundo progreso:', error);
                res.status(500).json({ error: 'Error al obtener las notas del segundo progreso' });
            } else {
                res.json(results);
            }
        }
